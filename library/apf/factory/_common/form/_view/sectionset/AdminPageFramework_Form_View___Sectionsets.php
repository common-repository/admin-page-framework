...
class AdminPageFramework_Form_View___Sectionsets extends AdminPageFramework_Form_View___Section_Base {
    ...
    public function get()
    {
        ...
        return $this->___getSpinnerOutput($_sOutput) . "<div id='{$_sElementID}' class='admin-page-framework-sectionsets admin-page-framework-form-js-on'>" . $_sOutput . $_oDebugInfo->get() . "</div>" ;
    }
    ...
    private function ___getSubSections($_aOutputs, $_sSectionsID, $_aSection, $_aSubSections)
    {
        if (! empty($_aSection[ 'repeatable' ])) {
            $_aOutputs[ 'section_contents' ][] = AdminPageFramework_Form_View___SectionRepeatableButtons::get($_sSectionsID, $_aOutputs[ 'count_subsections' ], $_aSection[ 'repeatable' ], $this->oMsg);
